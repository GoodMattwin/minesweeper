...
function placeBombs(numberOfBombs, bombBoard) {
  ...
  while (numberOfBombsPlaced < numberOfBombs) {
    var randomRowIndex = randomNumber(bombBoard.rows);
    var randomColumnIndex = randomNumber(bombBoard.columns);
...
function startGame(rows, columns, bombs) {
  var player = new buildBoard(rows, columns, ' ');
  var bomb = new buildBoard(rows, columns, null);
  console.log('Player: ');
  console.log(player.board);
  console.log('Bombs: ');
  console.log(bomb.board);
